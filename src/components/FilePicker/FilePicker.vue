                        ...
                        <view class="list-item">
                            <view class="left">
                                ...
                                <view class="info">
                                    ...
                                    <view v-if="isString(file.uuid)" class="size"> 已上传 </view>
...
const appendFiles = (files: any[]): any[] => {
    return Array.from(new Set([...uploadFiles.value, ...files].map((file) => file.name))).map((name) => {
...
const getAllowTypes = (kind: string) => {
    ...
    if (allowFileTypes[kind]) {
        (res.name = allowFileTypes[kind].name), (res.value = allowFileTypes[kind].value.join(','));
    }
    return res;
};
