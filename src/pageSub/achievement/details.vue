 ...
 * @LastEditors: Lowkey
 * @LastEditTime: 2024-05-23 17:08:32
    ...
    <view>
        ...
        <view class="content">
            ...
            >
                <view>
                    <view
                        v-for="(item, index) in dataState.listData"
        ...
        <fixed-btn :suggestion-params="suggestionParams" top="240rpx" />
    </view>
</template>
...
const suggestionParams = computed(() => ({
    ...
    suggestionType: 'grade',
}));
const { searchState, dataState, fetchList, refresh, loadMore, otherData, hasMore, isRefresh, loading, hasMoreLoading } = useRefreshList(searchParams, {
    immediate: false,
});
...
const handleListClick = (cur: Record<string, any>) => {
    const { courseid, itemType, instance, id } = cur;
    handleResourceClick({ modulename: itemType, id, instance }, courseid);
};
onLoad((option) => {
    if (option) {
