...
<template>
    <app-provider>
        ...
        <view class="container">
            ...
            <view class="content">
                <uni-title type="h4" align="left" style="padding-bottom: 40rpx;" title="重置初始化密码"></uni-title>
...
import {updatePassword, updatePhoneOrEmail, validRule} from '@/services/login';
...
const setPassword =async () => {
    const {code,message}=await validRule({characters:passwordForm.value.password});
    if(code===0){
        const curParams={
            'access_token':storage.get(StorageEnum.PORTAL_TOKEN),
            password:encryptSHA1(passwordForm.value.password),
        };
        const {code,message}=await updatePassword(curParams);
        if(code===0){
            router.replaceAll({
                name: 'Home',
            });
