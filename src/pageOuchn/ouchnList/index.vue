...
const formatCreateDate = (createDate) => {
    ...
    const date =
        new Date(createDate).getMonth() + 1 < 10
            ? `${new Date(createDate).getFullYear()}.0${new Date(createDate).getMonth() + 1}`
            : `${new Date(createDate).getFullYear()}.${new Date(createDate).getMonth() + 1}`;
    return { day, date };
};
