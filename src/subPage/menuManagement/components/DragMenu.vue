<template>
    <view ref="dragRef" class="l-drag l-class" :style="[areaStyles]" @touchstart="setDisabled">
        <movable-area v-if="isReset" class="l-drag__inner" :style="[innerStyles]">
            <slot></slot>
            <movable-view v-if="isDrag && props.ghost" key="l-drag-clone" class="l-drag__ghost" :animation="true" :style="[viewStyles]" direction="all" :x="ghostEl.x" :y="ghostEl.y">
...
interface GridRect extends Position{
	row : number
	// x : number
	// y : number
	x1 : number
...
interface Grid extends Position{
	id : string
	index : number
	oldindex : number
	content : any
...
/* #ifndef APP-NVUE */
.l-drag__delete::before,
.l-drag__handle::before {
  content: '';
  position: absolute;
  width: 100%;
  height: 100%;
