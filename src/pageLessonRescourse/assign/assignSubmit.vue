    ...
    <app-provider>
        ...
        <view v-if=" fileConfigs.enabled && maxfilesubmissions && maxsubmissionsizebytes">
            <uni-section title="文件提交" type="line" style="background-color: transparent;">
                <file-picker ref="fileRef" :upload-file-list="uploadFileList" :max-size="Number(maxsubmissionsizebytes)" :max-files="Number(maxfilesubmissions)" :allow-types="filetypeslist" />
            </uni-section>
        </view>
        <view class="btn">
...
const doSubmitAfterUpload = (response:any[])=>{
    ...
    if(response.length){
        ...
        useAssign.saveAssign(params,()=>{router.back();});
    }
};
