            ...
            <view v-if="!isEmpty(assignData)" class="content">
                ...
                <view v-if="assignData['gradingstatus'] === 'graded'" class="tab">
                    <uni-segmented-control
                        ...
                        style-type="text"
                        active-color="#2b83d7"
                        @click-item="(e:any)=>currentTab=e.currentIndex"
                    />
                </view>
                <view v-if="assignData['gradingstatus'] !== 'graded' || currentTab === 0">
                    ...
                    <view class="actions">
                        <button v-if="assignData.canedit" type="primary" @click="handleGoSubmitAssign">
                            {{ assignData.submitStatus === 'new' ? '提交作业' : '编辑提交的作业' }}
                        </button>
...
onShow(async () => {
    ...
    const assignParams = {
        ...
        userid,
        courseid,
        ssignId: instance, // ssignId 没错，后台漏写a
    };
